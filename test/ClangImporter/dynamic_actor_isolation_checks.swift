...
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -enable-upcoming-feature DynamicActorIsolation -Xllvm -sil-print-types -emit-silgen -module-name preconcurrency_conformances %t/src/checks.swift | %FileCheck %t/src/checks.swift
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -swift-version 6 -disable-dynamic-actor-isolation -Xllvm -sil-print-types -emit-silgen -module-name preconcurrency_conformances %t/src/checks_disabled.swift | %FileCheck %t/src/checks_disabled.swift
