...
// RUN: %target-swift-frontend %s -module-name Actor -clang-header-expose-decls=has-expose-attr -typecheck -verify -emit-clang-header-path %t/actor.h
// RUN: %FileCheck %s < %t/actor.h

// RUN: %check-interop-cxx-header-in-clang(%t/actor.h)

// RUN: %target-swift-frontend %s -module-name Actor -typecheck -verify -emit-clang-header-path %t/actor-public.h -enable-experimental-cxx-interop
// RN: %FileCheck %s < %t/actor-public.h
// RUN: %check-interop-cxx-header-in-clang(%t/actor-public.h)

// RUN: %target-swift-frontend %s -module-name Actor -enable-library-evolution -clang-header-expose-decls=has-expose-attr -typecheck -verify -emit-clang-header-path %t/actor-evo.h
