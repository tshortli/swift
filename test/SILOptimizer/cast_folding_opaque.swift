// RUN: %target-swift-frontend -enable-library-evolution -O -Xllvm -sil-print-types -emit-sil %s %S/Inputs/cast_folding_opaque_other.swift -module-name cast_folding_opaque
// RUN: %target-swift-frontend -enable-library-evolution -Onone -Xllvm -sil-print-types -emit-sil -primary-file %s %S/Inputs/cast_folding_opaque_other.swift -module-name cast_folding_opaque | %FileCheck %s
