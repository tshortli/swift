// RUN: %target-run-simple-swift( -parse-as-library) | %FileCheck %s --dump-input=always
// RUN: %target-run-simple-swift( -parse-as-library -swift-version 5 -strict-concurrency=complete -enable-upcoming-feature NonisolatedNonsendingByDefault) | %FileCheck %s --dump-input=always
