// RUN: %target-typecheck-verify-swift -parse-as-library -enable-experimental-feature SwiftRuntimeAvailability -target %target-swift-5.5-abi-triple
...
func availableInSwift99_0Runtime() { }
...
func alwaysAvailable() {
  ...
  availableInSwift99_0Runtime() // expected-error {{'availableInSwift99_0Runtime()' is only available in Swift 99.0 or newer}}
  // expected-note@-1 {{add 'if #available' version check}}{{3-32=if #available(Swift 99.0, *) {\n      availableInSwift99_0Runtime()\n  \} else {\n      // Fallback on earlier versions\n  \}}}
