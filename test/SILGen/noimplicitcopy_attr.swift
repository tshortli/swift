// RUN: %target-swift-emit-silgen -enable-experimental-move-only -parse-stdlib %s | %FileCheck %s
// RUN: %target-swift-emit-sil -enable-experimental-move-only -parse-stdlib %s | %FileCheck -check-prefix=CHECK-SIL %s
// RUN: %target-swift-emit-sil -O -enable-experimental-move-only -Xllvm -sil-disable-pass=FunctionSignatureOpts -parse-stdlib %s | %FileCheck -check-prefix=CHECK-SIL %s
