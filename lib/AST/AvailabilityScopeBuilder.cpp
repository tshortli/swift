...
private:
  ...
  AvailabilityQuery buildAvailabilityQuery(
    ...
    case AvailabilityDomain::Kind::Universal:
      ...
      // evaluates to "true" at compile time. Otherwise, generate a query for
      // the variant spec. For example, when compiling zippered for macOS, this
      // should generate a query that just checks the iOS version at runtime:
      //
      //    if #available(iOS 18, *) { ... }
      //
      if (variantRange)
        return buildAvailabilityQuery(*variantSpec, std::nullopt,
                                      isUnavailability);
